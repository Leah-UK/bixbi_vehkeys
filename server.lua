...
ESX.RegisterCommand('givekeys', 'superadmin', function(xPlayer, args, showError)
	TriggerEvent('bixbi_collection:AddKey', xPlayer.playerId, args.plate)
...
ESX.RegisterCommand('checkkeys', 'user', function(xPlayer, args, showError)
	TriggerEvent('bixbi_collection:AddKey', xPlayer.playerId, xPlayer.playerId, args.plate)
...
AddEventHandler('esx:playerLoaded', function(source, xPlayer)
    ...
    TriggerEvent('bixbi_collection:AddKey', source, source, jobName)
end)

local Keys = {}
RegisterServerEvent("bixbi_collection:CheckKey")
AddEventHandler("bixbi_collection:CheckKey", function(vehicle, plate, lockState, vehicleClass)
    plate = string.gsub(plate, "^%s*(.-)%s*$", "%1")
    local xPlayer = ESX.GetPlayerFromId(source)

    if ((xPlayer.job.name == 'police' or xPlayer.job.name == 'ambulance') and vehicleClass == 18) then
        TriggerClientEvent("bixbi_collection:ToggleLockAction", source, vehicle, lockState)
        return
    end
...
-- RegisterServerEvent("bixbi_collection:AddKey")
AddEventHandler("bixbi_collection:AddKey", function(targetId, plate, playerId)
...
-- RegisterServerEvent("bixbi_collection:RemoveKey")
AddEventHandler("bixbi_collection:RemoveKey", function(targetId, plate)
    plate = string.gsub(plate, "^%s*(.-)%s*$", "%1")
    print(plate)
...
ESX.RegisterServerCallback('bixbi_collection:UserKeys', function(source, cb)
    local list = {}
